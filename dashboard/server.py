...
def handle_create_task(title, org='规划智能体', official='规划智能体', priority='normal', template_id='', params=None, target_dept=''):
    """从看板创建新任务（圣旨模板下旨）。"""
    ...
    # 正确流程起点：用户 -> 协调智能体分拣
    # target_dept 记录模板建议的最终执行部门（仅供参考）
    initial_org = '协调智能体'
    new_task = {
        'id': task_id,
        'title': title,
    ...
    return {'ok': True, 'taskId': task_id, 'message': f'任务 {task_id} 已创建，正在派发给协调智能体'}
...
def handle_review_action(task_id, action, comment=''):
    ...
    task.setdefault('flow_log', []).append({
        'at': now_iso(),
        'from': '审议智能体' if task.get('state') != 'Done' else '皇上',
        'to': to_dept,
        'remark': remark
    })
...
_ORG_AGENT_MAP = {
    ...
    '派发智能体': 'dispatcher', '数据工程师': 'data_engineer', '生信工程师': 'bioinfo_engineer',
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 协调智能体自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
...
def handle_repair_flow_order():
    ...
    for task in tasks:
        ...
        first = flow_log[0]
        if first.get('from') != '用户' or first.get('to') != '协调智能体':
            continue
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    _msgs = {
        'coordinator': (
            ...
            f'任务ID: {task_id}\n'
            f'标题: {title}\n'
            f'请分析用户意图，判断是遍寻模式还是定向模式，然后转交规划智能体设计方案。'
        ),
        'planner': (
            f'📋 协调智能体已分拣，请设计方案\n'
            f'任务ID: {task_id}\n'
            f'标题: {title}\n'
            f'请设计分析方案，选择合适的 Genos API 和参数配置。'
        ),
        'reviewer': (
            f'🔍 规划智能体方案提交审议\n'
            f'任务ID: {task_id}\n'
...
def main():
    parser = argparse.ArgumentParser(description='GeneClaw 基因分析多智能体看板服务器')
    ...
    log.info(f'GeneClaw 看板启动 → http://{args.host}:{args.port}')
    print(f'   按 Ctrl+C 停止')
