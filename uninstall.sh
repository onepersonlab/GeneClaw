...
check_env() {
  ...
  echo ""
  echo -e "${YELLOW}确定要卸载「GeneClaw」系统并清理相关 Agent 数据吗？${NC}"
  read -p "(y/N) " -r
  echo
  if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    info "已取消卸载"
...
unregister_agents() {
  info "从 OpenClaw 移除 GeneClaw Agents 注册信息..."
...
remove_workspaces() {
  ...
  AGENTS=(coordinator planner reviewer dispatcher data_engineer bioinfo_engineer clinical_expert reporter_agent)
  removed=0
